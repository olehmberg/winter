	...
	SymmetricBlocker<RecordType, SchemaElementType, BlockedType, MatchableValue>
{
	...
	private VectorisedPairFilter<BlockedType, MatchableValue> pairFilter;
	...
	/**
	 * If specified, the filter will be applied to all pairs that are generated by this blocker.
	 * A {@link VectorisedPairFilter} receives a generated pair and all blocking key values that were generated for both records (the vecotrs).
	 * 
	 * @param vectorFilter the vectorFilter to set
	 */
	public void setPairFilter(VectorisedPairFilter<BlockedType, MatchableValue> vectorFilter) {
		this.pairFilter = vectorFilter;
	}
	...
	public Processable<Correspondence<BlockedType, MatchableValue>> runBlocking(
		...
		Processable<Pair<BlockedType, Set<String>>> blockingVectors1 = null;
		Processable<Pair<BlockedType, Set<String>>> blockingVectors2 = null;
		if(pairFilter!=null) {
			// optional: create blocking key value vectors for each BlockedType record (needed to calculate Jaccard/Cosine Similarity over blocking vectors)
			blockingVectors1 = grouped1.aggregate(
					(Block record, DataIterator<Pair<BlockedType, String>> resultCollector) 
					-> {
						for(Pair<BlockedType, Processable<Correspondence<MatchableValue, Matchable>>> blocked : record.getSecond().getElements()) {
							resultCollector.next(new Pair<>(blocked.getFirst(), record.getFirst()));
						}
					},
					new SetAggregator<BlockedType, String>());
			blockingVectors2 = grouped2.aggregate(
					(Block record, DataIterator<Pair<BlockedType, String>> resultCollector) 
					-> {
						for(Pair<BlockedType, Processable<Correspondence<MatchableValue, Matchable>>> blocked : record.getSecond().getElements()) {
							resultCollector.next(new Pair<>(blocked.getFirst(), record.getFirst()));
		...
		if(pairFilter!=null) {
			// aggregate the pairs
			result = result.aggregate(new IdentityMatchingRule<>(0.0), new VotingAggregator<>(true, 1, 0.0)).map((r,c)->c.next(r.getSecond()));
			
			// join the blocking vectors to the created pairs
			result = result
				.join(blockingVectors1, (c)->c.getFirstRecord(), (v)->v.getFirst())
				.join(blockingVectors2, (p)->p.getFirst().getSecondRecord(), (v)->v.getFirst())
				.map(
					(Pair<Pair<Correspondence<BlockedType, MatchableValue>, Pair<BlockedType, Set<String>>>, Pair<BlockedType, Set<String>>> record, DataIterator<Correspondence<BlockedType, MatchableValue>> resultCollector) 
					-> {
						Correspondence<BlockedType, MatchableValue> cor = pairFilter.createFinalPair(record.getFirst().getFirst(), record.getFirst().getSecond().getSecond(), record.getSecond().getSecond());

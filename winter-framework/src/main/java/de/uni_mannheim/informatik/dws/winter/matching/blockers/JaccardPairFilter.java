 ...
 *
 */
public class JaccardPairFilter<CorrespondenceType extends Matchable, CausalType extends Matchable> implements VectorisedPairFilter<CorrespondenceType, CausalType> {

	private double threshold = 0.0;
	
	public JaccardPairFilter(double threshold) {
	...
	public Correspondence<CorrespondenceType, CausalType> createFinalPair(
		...
		double intersection = pair.getSimilarityScore();
		
		double union = (double)Q.union(leftBlockingVector, rightBlockingVector).size();
		
		pair.setsimilarityScore(intersection / union);
